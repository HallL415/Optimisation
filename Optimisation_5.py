...
####################################### End of problematic region for time ######################################################
        ...
        # below the position array is not duplicated so it can't be used - comment it out
        # start_time = time.time()
        # V_perm_order0_new = np.ones(int(n/2))
        # V_perm_order1_new = np.ones(int(n/2))
        # value = 0 # start position value for the i=0 row
        # value2 = 2**(ii) # start position value for the i=1 row
        # for i in range(int(n/2)):
        #     if i == 0:
        #         V_perm_order0_new[i] = value
        #         V_perm_order1_new[i] = value2
        #     else:
        #         if (i) % (2**(ii)) == 0: # ii gives the number of times the indeces have shifted (happens every time step)
        #             value += int(2**(ii) + 1)
        #             value2 += int(2**(ii) + 1)
        #         else:
        #             value += 1
        #             value2 += 1
        #         V_perm_order0_new[i] = int(value)
        #         V_perm_order1_new[i] = int(value2)
                
        # V_perm_order0_new = V_perm_order0_new.repeat(2).reshape(-1)
        # V_perm_order1_new = V_perm_order1_new.repeat(2).reshape(-1)
...
################################################# new addition
 ...
 ##########################################################################################################################       
        ...
        for i in range(n): #n is how many permutations we have in total for a given L.
            ...
            for j in range(len(V_indices)):                        
                i_values[f'{V_indices[j]}'].append(permutation[int(i_pos[j])])
                
                i_values_U[f'{U_indices[j]}'].append(permutation[int(i_pos_U[j])])
